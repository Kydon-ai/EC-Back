...
export const uploadDocumentToRagflow = async (file, knowledgeBaseId) => {
	try {
		...
		throw new Error(
			`RAGFlow上传失败: ${error.response?.data?.message || error.message}`
		);
	}
};
...
export const removeDocumentFromRagflow = async docIds => {
	try {
		...
		throw new Error(
			`RAGFlow删除失败: ${error.response?.data?.message || error.message}`
		);
	}
};
...
export const createKnowledgeBaseToRagflow = async name => {
	try {
		...
		throw new Error(
			`RAGFlow创建知识库失败: ${error.response?.data?.message || error.message}`
		);
	}
};
...
export const removeKnowledgeBaseFromRagflow = async kbId => {
	try {
		...
		console.error(
			'Error removing knowledge base from RAGFlow:',
			error.response?.data || error.message
		);
		throw error;
	}
